...
def min_biomass(s,n,e,b):
    '''Return the square of the difference between the calculated and observed biomass with each state variable as an input,
    rather than as a series. This is used for the richness calculation. Minimize after dividing by observed biomass
    so that the range is reasonable.'''
    st = pd.Series({'S':s,'N':n,'E':e})
    bc = biomass(st)
    return ((bc - b)/b)**2

def richness(n,e,b):
    ''' From the abundance, metabolic rate, and biomass, get species richness.
    Minimize the function min_biomass. Do this on a bounded range for s from 4 to n'''
    # Guess s0 to be either n/100 or 10, depending on which is larger.
    s0 = np.max([int(n/100),10])
    sol = minimize_scalar(min_biomass,args=(n,e,b),bounds=(4,int(n)),method='bounded')
    return sol.x